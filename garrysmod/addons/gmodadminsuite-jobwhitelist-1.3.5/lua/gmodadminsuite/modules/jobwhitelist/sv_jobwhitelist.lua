...
GAS:hook("ShowSpare2", "jobwhitelist:FunctionMenuKey:F6", function(ply)
	if (GAS.JobWhitelist.Config.FunctionMenuKey == "F6") then
		ply:ConCommand("gmodadminsuite jobwhitelist")
	end
end)
					...
					if (is_blacklist) then
						if (not GAS.JobWhitelist:IsBlacklistEnabled(job_index)) then
							...
							if (args[1] == "add") then
								...
								GAS.JobWhitelist:RemoveFromBlacklist(job_index, GAS.JobWhitelist.LIST_TYPE_STEAMID, account_id)
								GAS:print("[JobWhitelist] Removed from blacklist successfully", GAS_PRINT_COLOR_GOOD)
							end
						end
					else
						if (not GAS.JobWhitelist:IsWhitelistEnabled(job_index)) then
							...
							if (args[1] == "add") then
								...
								GAS.JobWhitelist:RemoveFromWhitelist(job_index, GAS.JobWhitelist.LIST_TYPE_STEAMID, account_id)
								GAS:print("[JobWhitelist] Removed from whitelist successfully", GAS_PRINT_COLOR_GOOD)
							end
						end
					end
				end
