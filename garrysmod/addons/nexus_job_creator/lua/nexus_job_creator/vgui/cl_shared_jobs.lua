...
function PANEL:Init()
    ...
    for id, v in pairs(Nexus.JobCreator.ActiveJobs) do
        if not v.Players[LocalPlayer():SteamID64()] then continue end
        table.insert(ourJobs, v)
    end
    
    for i = 1, (math.max(Nexus:GetValue("nexus-jobcreator-max-sharedjobs"), #ourJobs)) do
        ...
        jobPanel:SetShared(true)
    end
end

function PANEL:Paint(w, h)
end
vgui.Register("Nexus:JobCreator:SharedJobs", PANEL, "EditablePanel")