...
function PANEL:FillPanel()
    ...
    local textX, textY = surface.GetTextSize( "Create Item" )
    ...
    createNewButton.Paint = function( self2, w, h )
        ...
        draw.SimpleText( "Create Item", "BRICKS_SERVER_Font20", 12+iconSize+5, h/2, BRICKS_SERVER.Func.GetTheme( 6, 20+(235*(alpha/255)) ), 0, TEXT_ALIGN_CENTER )
    end
    createNewButton.DoClick = function()
        BS_ConfigCopyTable.UNBOXING.Items[BRICKS_SERVER.Func.ConfigGenerateUnboxingID( "ITEM" )] = {
            Name = "New Item",
        ...
        BRICKS_SERVER.Func.CreateTopNotification( "New item successfully created!", 3, BRICKS_SERVER.DEVCONFIG.BaseThemes.Green )
    end
...
function PANEL:Refresh()
    ...
    for k, v in pairs( BS_ConfigCopyTable.UNBOXING.Items or {} ) do
        ...
        slotBack:FillPanel( { "ITEM_" .. k, v }, 1, function()
            self.popoutPanel = vgui.Create( "bricks_server_config_unboxing_items_popup", self )
            ...
            self.popoutPanel:SetItemTable( k, v, function( valueChanged, newItemTable )
                if( valueChanged ) then
                    BS_ConfigCopyTable.UNBOXING.Items[k] = newItemTable
                    BRICKS_SERVER.Func.ConfigChange( "UNBOXING" )
                    ...
                    BRICKS_SERVER.Func.CreateTopNotification( "Item successfully edited!", 3, BRICKS_SERVER.DEVCONFIG.BaseThemes.Green )
                end
            end, function() 
                BRICKS_SERVER.Func.ConfigRemoveUnboxingItem( "ITEM", k )
                self:Refresh()
                BRICKS_SERVER.Func.CreateTopNotification( "Item successfully removed!", 3, BRICKS_SERVER.DEVCONFIG.BaseThemes.Red )
            end, function() 
                BS_ConfigCopyTable.UNBOXING.Items[BRICKS_SERVER.Func.ConfigGenerateUnboxingID( "ITEM" )] = v
                BRICKS_SERVER.Func.ConfigChange( "UNBOXING" )

                self:Refresh()
                BRICKS_SERVER.Func.CreateTopNotification( "Item successfully duplicated!", 3, BRICKS_SERVER.DEVCONFIG.BaseThemes.Green )
            end )
        end )
    end
end
...
vgui.Register( "bricks_server_config_unboxing_items", PANEL, "DPanel" )