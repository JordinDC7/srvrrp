...
function PANEL:FillPanel()
    ...
    local textX, textY = surface.GetTextSize( "Create Key" )
    ...
    createNewButton.Paint = function( self2, w, h )
        ...
        draw.SimpleText( "Create Key", "BRICKS_SERVER_Font20", 12+iconSize+5, h/2, BRICKS_SERVER.Func.GetTheme( 6, 20+(235*(alpha/255)) ), 0, TEXT_ALIGN_CENTER )
    end
    createNewButton.DoClick = function()
        BS_ConfigCopyTable.UNBOXING.Keys[BRICKS_SERVER.Func.ConfigGenerateUnboxingID( "KEY" )] = {
            Name = "New Key",
        ...
        BRICKS_SERVER.Func.CreateTopNotification( "New key successfully created!", 3, BRICKS_SERVER.DEVCONFIG.BaseThemes.Green )
    end
...
function PANEL:Refresh()
    ...
    for k, v in pairs( BS_ConfigCopyTable.UNBOXING.Keys or {} ) do
        ...
        slotBack:FillPanel( { "KEY_" .. k, v }, 1, function()
            ...
            self.popoutPanel:SetItemTable( k, v, function( valueChanged, newItemTable )
                if( valueChanged ) then
                    ...
                    BRICKS_SERVER.Func.CreateTopNotification( "Key successfully edited!", 3, BRICKS_SERVER.DEVCONFIG.BaseThemes.Green )
                end
            end, function() 
                BRICKS_SERVER.Func.ConfigRemoveUnboxingItem( "KEY", k )
                self:Refresh()
                BRICKS_SERVER.Func.CreateTopNotification( "Key successfully removed!", 3, BRICKS_SERVER.DEVCONFIG.BaseThemes.Red )
            end, function() 
                BS_ConfigCopyTable.UNBOXING.Keys[BRICKS_SERVER.Func.ConfigGenerateUnboxingID( "KEY" )] = v
                BRICKS_SERVER.Func.ConfigChange( "UNBOXING" )

                self:Refresh()
                BRICKS_SERVER.Func.CreateTopNotification( "Key successfully duplicated!", 3, BRICKS_SERVER.DEVCONFIG.BaseThemes.Green )
            end )
        end )
    end
end
...
vgui.Register( "bricks_server_config_unboxing_keys", PANEL, "DPanel" )