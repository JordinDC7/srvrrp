/*
    Addon id: 64edeaec-8955-454a-aac4-1d19d72ee4af
    Version: v1.6.5 (stable)
*/

local ITEM = BRICKS_SERVER.Func.CreateItemType("zgo2_seed")
                                                                                                                                                                                                                                                                                                                                                                                                                                                       -- 4c5b22d0c464981a2a2cb55407ea8523b955c581541c9aaa45b7d50e2253186c

ITEM.GetItemData = function(ent)
	if not IsValid(ent) then return end
	if not ent.GetPlantID then return end

	local itemData = { "zgo2_seed", "models/zerochain/props_growop2/zgo2_weedseeds.mdl", zgo2.Plant.GetID(ent:GetPlantID()), math.Round(ent:GetCount()) }

	return itemData, 1
end
                                                                                                                                                                                                                                                                                                                                                                                                                                                       -- 76561199109663690
                                                                                                                                                                                                                                                                                                                                                                                                                                                       -- 76561199109663690

ITEM.OnSpawn = function(ply, pos, itemData, itemAmount)
	if not zgo2.Plant.IsValid(itemData[ 3 ]) then
		zclib.Notify(ply, zgo2.language[ "InvalidPlantData" ], 1)

		return false
	end

	local ent = ents.Create("zgo2_seed")
	if not IsValid(ent) then return end
	ent:SetPos(pos)
	ent:Spawn()
	ent:Activate()
	zclib.Player.SetOwner(ent, ply)
	ent:SetPlantID(zgo2.Plant.GetListID(itemData[ 3 ]))
	ent:SetCount(itemData[ 4 ])
end

ITEM.GetInfo = function(itemData)
	if not itemData then return {"",""} end
	return { zgo2.Plant.GetName(itemData[ 3 ]) .. " " .. tostring(itemData[ 4 ]) .. "x", "Some weed seeds.", "" }
end

local ang = Angle(0, -45, 0)
                                                                                                                                                                                                                                                                                                                                                                                                                                                       -- 4c5b22d0c464981a2a2cb55407ea8523b955c581541c9aaa45b7d50e2253186c

ITEM.ModelDisplay = function(Panel, itemtable)
	if not Panel.Entity or not IsValid(Panel.Entity) then return end
	local mn, mx = Panel.Entity:GetRenderBounds()
	local size = 0
	size = math.max(size, math.abs(mn.x) + math.abs(mx.x))
	size = math.max(size, math.abs(mn.y) + math.abs(mx.y))
	size = math.max(size, math.abs(mn.z) + math.abs(mx.z))
	Panel:SetFOV(20)
	Panel:SetCamPos(Vector(size, size * 3, size * 2))
	Panel:SetLookAt((mn + mx) * 0.5)
	Panel.Entity:SetAngles(ang)
	local WeedData = zgo2.Plant.GetData(itemtable[ 3 ])
	if not WeedData then return end
	zgo2.Seed.UpdateMaterial(Panel.Entity, WeedData, false)
end

ITEM.CanCombine = function(itemData1, itemData2) return false end
ITEM:Register()
                                                                                                                                                                                                                                                                                                                                                                                                                                                       -- 76561199109663699

if BRICKS_SERVER and BRICKS_SERVER.CONFIG and BRICKS_SERVER.CONFIG.INVENTORY and BRICKS_SERVER.CONFIG.INVENTORY.Whitelist then
	BRICKS_SERVER.CONFIG.INVENTORY.Whitelist[ "zgo2_seed" ] = { true, true }
end
