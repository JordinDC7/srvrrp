...
/*
	Defines how many entities of the specified class can be spawned / bought from the tablet
*/
zgo2.config.ShopSpawnLimit = {
	["zgo2_seedlibary"] = 1,
	["zgo2_seed"] = 5,
	["zgo2_pot"] = 12,
	["zgo2_rack"] = 6,
...
*/
zgo2.config.SelectedLanguage = "en"
...
*/
zgo2.config.CollisionFix = false
...
if GAMEMODE and GAMEMODE.Config and GAMEMODE.Config.PocketBlacklist then
	GAMEMODE.Config.PocketBlacklist[ "zgo2_npc" ] = true
	GAMEMODE.Config.PocketBlacklist[ "zgo2_npc_export" ] = true
	GAMEMODE.Config.PocketBlacklist[ "zgo2_plant" ] = true
...
zgo2.config.Plant = {
	...
	// If set then a plant will only spawn this amount of weedbranches on harvest
...
zgo2.config.Backpack = {
	...
	// Should the backpack be dropped on death if it had items inside?
	DropOnDeath = true,

	// How long until the dropped backpack entity should despawn
	DespawnTime = 120,

	// How many slots does the backpack have?
	SlotCount = 24,

	// Should the backpack model be visible on players which have the backpack swep?
	DrawModel = true,

	// Those classes are allowed to be picked up
	// NOTE Classes registerd at lua/zgo2/backpack/items/ will be automaticly added to this list
	Allowed = {
		//"prop_",
	},

	// Those classes are not allowed to be picked up
	Blocked = {
		"edit_sky",
	},
	...
	Save = {
		...
		// Should we save the players backpack?
		Enabled = true,
...
/*
	The seedlibary can be used to store weed seeds
*/
zgo2.config.SeedLibary = {

	// Do we allow the players to reset the inactivity timer for the spliced weed seeds inside their seed libary?
	InactivityReset = true,

	// Any savefile which has not been modified for 2 months will be removed
	LifeTime = 5260000,
}
